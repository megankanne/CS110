...
HandleGetRequest(int connfd, char *path, char *version, char **imgFileNames, int numImgFiles)
{
	...
	char *msg1 =
		...
		"function IsEmpty(){\n"
		  "var regex = /^[a-z]+$/i;"
		  "var inputw = document.forms['input'].word.value;"
		  "if(!regex.test(inputw))\n"
		  "{\n"
		    "alert('Entry must be at least one character and consist of only letters.');\n"
		    "return false;\n"
		  "}\n"
		    "return true;\n"
		"}\n"
	...
	char *msg2 = 
		...
		"<input type='submit' value='Go!' onclick='return IsEmpty();'><br>\n<br>\n"
